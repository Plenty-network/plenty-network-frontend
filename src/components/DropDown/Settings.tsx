...
export function SettingsDropdown(props: IDropdownProps) {
  ...
  return (
    <div className={`relative min-w-[76px]  ${props.className}`} ref={reff}>
      <div
        className={clsx(
          "bg-text-800/[0.25]  cursor-pointer flex gap-2 md:gap-2 py-1 px-2 md:px-2 md:justify-between border  rounded-lg hover:border-text-700 hover:bg-text-800/[0.25]",
          isDropDownActive
      ...
      >
        <p className={clsx(" flex gap-1", isDropDownActive ? "text-white" : "text-text-600")}>
          {props.selectedText && props.selectedText.length ? (
            <span className="text-white font-body3 ">{props.selectedText}</span>
          ) : (
            <>
              <span className="hidden md:block  md:font-caption1">
                {props.title && props.title}
              </span>{" "}
              <span className="font-subtitle1 md:hidden md:font-caption1">
                {props.title && props.title}
              </span>
            </>
          )}
  ...
  function Options(props: { onClick: Function; text: string }) {
    return (
      <div
        ...
        className=" hover:bg-muted-500 px-2 flex items-center h-[36px] z-10 cursor-pointer font-body3"
      >
        {props.text}
      </div>
    );
