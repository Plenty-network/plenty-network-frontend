...
export function PoolsTextWithTooltip (props: IPoolsTextWithTooltipProps) {
  const isTEZorCTEZ=(a:string)=> a === 'tez' || a === 'ctez';
    return (
      <ToolTip
      position={Position.top}
      toolTipChild={<div>
          <div className='text-text-500 text-f14 font-normal flex gap-1' >
              <div className={`text-white font-medium pr-1 ${isTEZorCTEZ(props.token1Name) ?'uppercase':''}`}>{props.token1}</div>
              <div>{props.token1Name}</div>
          </div>
          <div className='text-text-500 text-f14 font-normal flex gap-1' >
              <div className={`text-white font-medium pr-1 ${isTEZorCTEZ(props.token2Name) ?'uppercase':''}`}>{props.token2}</div>
