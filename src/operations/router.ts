...
export const routerSwap = async (
    ...
    try {
      ...
      for (let i = 0; i < path.length - 1; i++) {
        ...
        const minOut = minimum_Out_All[i].multipliedBy(Math.pow(10 , TOKEN[path[i+1]].decimals)).toFixed(0);
      ...
      let swapAmount = amount.multipliedBy(Math.pow(10, TOKEN_IN.decimals)).toFixed(0);
      ...
      if (tokenInCallType === TokenType.TEZ) {
        ...
        if (tokenInCallType === TokenType.FA12) {
          batch = Tezos.wallet
            ...
            .withContractCall(routerInstance.methods.routerSwap(DataMap, swapAmount, recipent));
        } else {
            // FA2 Call
          batch = Tezos.wallet
            .batch()
